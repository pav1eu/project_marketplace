...
def smartphone_category(samsung_product):
    return Category("Смартфоне", "Смартфоны, как средство не только коммуникации", [samsung_product])


def test_smartphone_category_init(smartphone_category):
    assert smartphone_category.name == "Смартфоне"
    assert smartphone_category.description == "Смартфоны, как средство не только коммуникации"
